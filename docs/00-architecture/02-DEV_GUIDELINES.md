...
### 5.1 声明方式（entry point）

每个 starter 必须注册 `pyspring.starters` entry point，指向返回 `StarterDeclaration` 的加载函数：
...
加载函数返回 `StarterDeclaration`（`name` / `scan_packages` / `auto_configuration` / `order` / `requires`）：
...
- [ ] 新增代码符合本节命名/包划分/接口规范
- [ ] starter 注册了 `pyspring.starters` entry point
