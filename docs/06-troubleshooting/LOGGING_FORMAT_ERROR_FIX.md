...
如果使用 PySpring v1.1.0+，也可以安全使用 `{extra[file_relative]}`。

### Q2: 我能使用 `extra[file_relative]` 吗？

A: 可以！从 PySpring v1.1.0 开始，`file_relative` 字段会自动添加，无需担心 KeyError。

### Q3: 为什么会出现 `KeyError: 'session_id'`？

A: 在旧版本（< v1.1.0）中，这是因为：

1. 日志格式中使用了 `{extra[session_id]}`
2. 但代码中没有使用 `logger.bind(session_id="xxx")`
3. Loguru 尝试访问不存在的字段导致 KeyError

✅ **解决方法（PySpring v1.1.0+）**：
...
- 方案 1: 移除格式中的 `{extra[session_id]}`
- 方案 2: 在代码中提供该字段：`logger.bind(session_id="xxx")`
- 方案 3: 升级到 v1.1.0+（推荐）
...
A: 当然可以！PySpring v1.1.0+ 支持任何自定义字段。
...
- **2026-01-23 v1.1.0**: 🎉 **重大更新** - 引入 `SafeExtraDict`，支持任意自定义字段，彻底解决 KeyError 问题
    - 实现智能字典，访问不存在的键返回空字符串而非抛出异常
    - 完全通用的解决方案，无需预定义字段列表
    - 性能优化，按需处理而非预填充
- **2026-01-23**: 修复 `_add_relative_path` 过滤器，确保 `extra` 字典和 `file_relative` 字段始终存在
- **2026-01-23**: 更新所有配置模板，使用安全的日志格式
- **2026-01-23**: 增强异常处理，避免在访问 `record["file"]` 时出错
