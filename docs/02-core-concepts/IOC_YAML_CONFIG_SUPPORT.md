...
from pyspring.ioc.annotations.bean import Bean
from pyspring.ioc.annotations.configuration import Configuration
...
class CacheConnectionInitializer:
    def __init__(self, cache_manager: CacheManagerService):
        # 手动加载配置
        config_mgr = RepositoriesConfigManager()
        cache_config = config_mgr.get('cache', {})
...
- ✅ **支持 YAML**：通过 ConfigLoader 和各种 ConfigManager
- ❌ **不够自动**：需要手动加载和传递
...
1. **先实现方案 A**（扩展 ConfigSection）
2. **更新 Cache 模块**作为试点
3. **验证后推广**到其他模块
4. **逐步删除**旧的 ConfigManager

这样就能实现 **Pydantic + YAML + 环境变量** 的完美融合！ 🚀
