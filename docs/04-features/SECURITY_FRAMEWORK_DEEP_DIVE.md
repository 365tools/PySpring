...
class WorkingHoursValidator(ISecurityContextValidator, ISingletonService):
    @property
    def name(self) -> str:
...
class DeviceTrustValidator(ISecurityContextValidator, ISingletonService):
    @property
    def name(self) -> str:
...
class IPRestrictionValidator(ISecurityContextValidator, ISingletonService):
    """
    IP 访问控制策略
...
class SubscriptionPlanValidator(ISecurityContextValidator, ISingletonService):
    """
    订阅权益注入策略
...
2. 继承了 `ISingletonService` (或使用 `@component`/`@service` 装饰器)。
...
class MyCustomPolicy(ISecurityContextValidator, ISingletonService):
    # 实现逻辑...
    pass
```
