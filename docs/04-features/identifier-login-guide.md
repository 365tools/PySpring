...
框架支持使用 `identifier` 字段进行登录，可以自动匹配以下字段：
...
from pyspring.core.ioc.di.inject import Inject
...
- 推荐使用 `identifier` 统一字段
- 同时支持 `user_id` 或 `email` 等具体字段
...
A: 不是必须的。`user_id` 和 `email` 字段同样支持。但推荐新项目使用 `identifier` 统一字段。
