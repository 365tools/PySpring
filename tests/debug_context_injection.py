...
from src.pyspring.security.authentication.impl.core.context import SecurityContextManagerService
...
# 检查服务名
expected_name = manager.generate_name(SecurityContextManagerService)
print(f"预期服务名: {expected_name}")

if expected_name in manager._registered_services:
    print(f"✅ 服务 {expected_name} 已注册")
    instance = manager.get(SecurityContextManagerService)
...
from src.pyspring.security.authentication.impl.session.login import LoginService
