...
def test_scanner_optimizations():
    """测试扫描器优化功能"""
    print("\n🔍 测试扫描器优化功能...")
    ...
    # 测试多次获取同一服务（验证缓存是否正常工作）
    for i in range(3):
        service = app_context.get_by_type(ConcreteService)
        print(f"✅ 第{i+1}次获取服务成功: {type(service).__name__}")
    
    print("🎉 扫描器优化测试完成！")


if __name__ == "__main__":
    test_scanner_optimizations()