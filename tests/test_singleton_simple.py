...
try:
    ...
    spec = __import__('importlib.util').util.spec_from_file_location(
        "ISingletonService",
        project_root / "src/pyspring/interfaces/ISingleton.py"
    )
...
try:
    singleton_classes = [
        ("SecurityConfigManager", project_root / "src/pyspring/security/auth/config_manager.py"),
        ("AuthenticationChainManager", project_root / "src/pyspring/security/auth/chain.py"),
        ("JWTEncryptionManager", project_root / "src/pyspring/security/auth/encryption.py"),
        ("LoggingConfigManager", project_root / "src/pyspring/log/loguru/config_manager.py"),
        ("RepositoriesConfigManager", project_root / "src/pyspring/repositories/config_manager.py"),
        ("ConfigRegistry", project_root / "src/pyspring/system/config/registry.py"),
        ("EnvConfigLoader", project_root / "src/pyspring/system/env.py"),
...
try:
    ...
    result = subprocess.run(
        ['powershell', '-Command',
         'Get-ChildItem -Path "d:\\Project\\PycharmProjects\\PySpring\\src" -Filter "*.py" -Recurse | '
