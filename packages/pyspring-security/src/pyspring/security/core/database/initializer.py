...
class AuthConfigService(IManaged):
    ...
    def __init__(self, db_manager: DBManagerService, security_config: SecurityConfigManager):
        """
        初始化认证配置服务
    ...
    async def init_tables(self) -> bool:
        ...
        try:
            ...
            async with engine.begin() as conn:
                ...
                if all(table in existing_tables for table in required_tables):
                    ...
                    try:
                        ...
                        logger.debug(f"[Success] 认证表已存在且可用, 跳过创建: {', '.join(required_tables)}")
                        return True
                    except Exception as verify_error:
                        logger.error(f"[Error] 认证表存在但不可用: {verify_error}")
                        raise RuntimeError(f"认证表存在但不可用, 可能结构损坏: {verify_error}")

                # 表不存在或部分表缺失, 创建所有表
                missing_tables = [t for t in required_tables if t not in existing_tables]
                if missing_tables:
                    logger.info(f"[Note] 检测到缺失的表: {', '.join(missing_tables)}, 开始创建...")

                await conn.run_sync(Base.metadata.create_all)
                logger.info(f"[Success] 认证表创建成功: {', '.join(required_tables)}")
            ...
            logger.error(f"[Error] 认证表创建失败: {e}")
