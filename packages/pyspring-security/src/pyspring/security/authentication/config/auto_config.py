...
class AuthenticationConfiguration:
    ...
    def default_login_providers(
        self, default_password_login_provider: DefaultPasswordLoginProvider
    ) -> list[ILoginProvider]:
        """
        组装默认的登录认证提供者列表
    ...
    def authentication_providers(
        self, config_manager: SecurityConfigManager, token_service: ITokenService
        ...
        providers = AuthProviderFactory.create_providers_from_config(
            token_manager=token_service, config_manager=config_manager
        )
        if not providers:
