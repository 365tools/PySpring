...
class TokenService(ITokenService):
    ...
    def __init__(self):
        """初始化 Token 服务（懒加载依赖）"""
        self._token_generator: (ITokenGenerator) | None = None
        self._cache: (CacheManagerService) | None = None
        self._db: (DBManagerService) | None = None
    ...
    def token_generator(self) -> ITokenGenerator:
        ...
        assert self._token_generator is not None
    ...
    async def create_refresh_token(self, data: dict[str, Any], expires_delta: (Any) | None = None) -> str:
        ...
        try:
            # 从 payload 获取用户标识符
            user_id: str | None = payload.get("sub")  # JWT 标准字段：user_id (UUID)

            exp_timestamp = decoded.get("exp")
            if exp_timestamp is not None:
                expires_at = datetime.fromtimestamp(int(exp_timestamp), UTC)
            else:
    ...
    async def revoke_token(self, token: str, reason: str = "") -> bool:
        ...
        try:
            ...
            # 获取用户标识符
            user_id: str | None = payload.get("sub")  # JWT 标准：user_id (UUID)

            exp_timestamp = payload.get("exp")
            if exp_timestamp is not None:
                expires_at = datetime.fromtimestamp(float(exp_timestamp), UTC)
            else:
                expires_at = datetime.now(UTC)
    ...
    async def revoke_user_refresh_tokens(
        ...
        try:
            ...
            async with await self.db.session() as db_session:
                ...
                try:
                    for record in active_tokens:
                        refresh_key = f"token:refresh:{user_id}:{record.token[:16]}"
