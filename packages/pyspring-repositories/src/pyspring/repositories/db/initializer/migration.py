...
class MigrationInitializer(IStartupInitializer):
    ...
    def _remove_duplicate_base_class_tables():
        ...
        策略：
        - 检查是否有多个表继承同一个抽象基类（如 BaseUserTable）
        - 保留用户自定义的表，移除框架内置的表
        - 识别规则：pyspring_ 前缀的是框架表
        - 该逻辑仅依赖 repositories 层定义的抽象基类，不反向依赖任何 starter 的具体表
        """
        try:
                    ...
                    if local_table_name and local_table_name == table.name:
                        ...
                        for base_class in abstract_bases:
                            if issubclass(orm_class, base_class) and orm_class != base_class:
                                tables_by_base.setdefault(base_class, []).append((table_name, orm_class))
                        break

            # 处理冲突：如果有多个表继承同一抽象基类，移除框架表（pyspring_ 前缀）
            removed_count = 0
