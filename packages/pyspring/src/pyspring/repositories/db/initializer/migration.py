...
class MigrationInitializer(IStartupInitializer):
    ...
    def _load_framework_models(self):
        """加载框架内置的 ORM 模型"""
        try:
    ...
    def _load_user_models(self) -> bool:
        """
        加载用户项目的 ORM 模型
    ...
    def _apply_table_prefix(self, prefix: Optional[str]):
        """
        为所有表应用前缀
    ...
    def _remove_duplicate_base_class_tables(self):
        """
        移除与用户自定义表冲突的框架表
        ...
        try:
            ...
            # 定义基类到框架表名的映射
            base_class_map = {
                BaseUserTable: 'pyspring_user',
                BaseRoleTable: 'pyspring_role',
                BasePermissionTable: 'pyspring_permission',
                BaseUserRoleTable: 'pyspring_user_role',
                BaseRolePermissionTable: 'pyspring_role_permission',
            }
