...
class CheckReferencesCommand(BaseCommand):
    ...
    arguments = [
        ...
        CommandArg(
            "--fix", action="store_true", help="Attempt to automatically fix missing imports for standard libraries"
        ),
        CommandArg("--strict", action="store_true", help="Fail if any issues are found (including Mypy issues)"),
        CommandArg(
            "--whitelist",
            help='Pipe-separated list of issue categories to include (e.g. "Static Method|Type Mismatch")',
        ),
    ]
...
class CheckBasedPyrightCommand(BaseCommand):
    ...
    arguments = [
        ...
        CommandArg(
            ...
            default="",
            metavar="RULE1,RULE2",
            help='Only report specific pyright rules, comma-separated (e.g. "reportDeprecated,reportExplicitAny")',
        ),
    ]
...
class CheckEncodingCommand(BaseCommand):
    ...
    arguments = [
        ...
        CommandArg(
            "--fix", action="store_true", help="Automatically fix encoding issues (convert to utf-8 without BOM)"
        ),
    ]
...
class CheckCircularCommand(BaseCommand):
    ...
    arguments = [CommandArg("path", nargs="?", default=".")]
...
class CheckExplicitCommand(BaseCommand):
    ...
    arguments = [CommandArg("path", nargs="?", default="."), CommandArg("--fix", action="store_true")]
...
class CheckLiftCommand(BaseCommand):
    ...
    arguments = [CommandArg("target", nargs="?", default="."), CommandArg("--fix", action="store_true")]
...
class CheckValidateCommand(BaseCommand):
    ...
    description = (
        "Comprehensive imports check. Detects missing modules (ModuleNotFoundError) and can auto-resolve broken paths."
    )
    arguments = [
        CommandArg("target", nargs="?", default="."),
...
class CheckResetCommand(BaseCommand):
    ...
    arguments = [
        CommandArg("path", nargs="?", default="."),
        CommandArg(
            ["-f", "--force"], action="store_true", help="Force delete satisfying imports as well (nuclear option)"
        ),
