...
class ImportResetVisitor(ast.NodeVisitor):
    ...
    def _should_remove(self, module_name: str | None, lineno: int) -> bool:
        if not module_name:
            return False
...
class ReconstructChecker(BaseChecker):
    ...
    def __init__(self, target_path):
        ...
        try:
            ...
            if file_path:
                lib_paths.append(os.path.dirname(file_path))
            lib_paths.extend(str(p) for p in getattr(pyspring, "__path__", []) or [])
