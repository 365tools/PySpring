...
class EncodingChecker(BaseChecker):
        ...
        if fix:
            if "Corrupted" in str(issue_type):
                self.add_issue(
                    file_path,
                    line_no,
                    f"{msg} -> Cannot auto-fix: File contains replacement characters. Manual repair required.",
                    level="error",
                )
    ...
    def _detect_issue(self, file_path: str) -> Tuple[bool, Optional[str], str, int, str]:
        """
