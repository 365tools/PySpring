...
def get_template_dir() -> Path:
    ...
    try:
        ...
        if file_path:
            search_paths.append(Path(file_path).parent)
        for p in getattr(pyspring, "__path__", []) or []:
...
def create_database_scripts(target_dir: Path):
    ...
    try:
        ...
        PermissionTable = orm_tables_mod.PermissionTable
        UserRoleTable = orm_tables_mod.UserRoleTable
        RolePermissionTable = orm_tables_mod.RolePermissionTable
        RefreshTokenTable = orm_tables_mod.RefreshTokenTable
