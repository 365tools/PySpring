...
from pyspring_cli.core.ui.console import Colors, print_header, print_success, print_info, print_warning, print_error, print_title
from pyspring_cli.core.ui.help import print_standard_command_help
...
def create_pyproject_toml(target_dir: Path, force: bool = False):
    ...
    # Remove pyspring CLI entry point (legacy)
    content = content.replace(
        '[project.scripts]\npyspring = "pyspring.cli:main"\n\n',
        ''
    )
    # Also remove any remaining CLI entry points
    content = content.replace(
        'pyspring = "pyspring.cli:main"',
        '# pyspring = "pyspring.cli:main"  # CLI moved to separate package'
    )
