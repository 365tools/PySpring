...
使用方式（推荐IoC注入）:
    ...
    class MyService:
        def __init__(self, config_service: SystemService):
            ...
            # 各 starter 配置通过独立注入（DatabaseConfig 等），而非 AppSettings 聚合
"""
...
class SystemService(IManaged):
    ...
    def get_yaml_config(self, filename: str, key: (str) | None = None, default: Any = None) -> Any:
        """
        从YAML配置文件中读取配置
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            # 认证配置由 security starter 独立注入校验，不再经 AppSettings 聚合
