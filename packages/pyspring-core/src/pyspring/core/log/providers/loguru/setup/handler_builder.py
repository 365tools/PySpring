...
class HandlerBuilder:
    ...
    @classmethod
    def build_console_handler(cls, config: dict[str, Any], level: str, advanced: dict[str, Any]) -> None:
        """
        构建控制台handler
    ...
    @classmethod
    def build_file_handler(cls, config: dict[str, Any], level: str, advanced: dict[str, Any]) -> None:
        """
        构建文件handler
        ...
        logger.add(
            str(file_path),
            format=config.get("format", "{time:YYYY-MM-DD HH:mm:ss.SSS} | {level: <8} | {message}"),
            level=level,
    ...
    def setup_all_handlers(
        cls, console_config: dict[str, Any], file_config: dict[str, Any], level: str, advanced: dict[str, Any]
    ) -> None:
        """
        设置所有handlers
