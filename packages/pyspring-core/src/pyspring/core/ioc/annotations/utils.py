...
def is_abstract_type(target: type) -> bool:
    ...
    示例:
        ...
        False
    """
    return inspect.isabstract(target) or isinstance(target, ABCMeta) or getattr(target, "_is_protocol", False)


def set_pyspring_attribute(target: type[T] | Callable[..., object], attr_name: str, value: object) -> None:
    """
    设置 PySpring 内部属性
...
def create_flexible_decorator(
    ...
    def decorator_impl(cls_or_none: type[T] | None = None, **kwargs: object) -> type[T] | Callable[[type[T]], type[T]]:
        """实际的装饰器实现"""
...
def create_component_decorator(component_type: str) -> Callable[..., object]:
    """
