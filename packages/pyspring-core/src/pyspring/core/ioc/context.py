...
提供容器访问点：每次 initialize() 创建独立上下文实例，
并记录为"最近一次初始化"，便于便捷访问。
"""
...
class ApplicationContext:
    ...
    每次 `initialize()` 都会创建**独立的上下文实例**（不再强制全局单例），
    避免多次初始化时容器被覆盖的问题。`get_instance()` 作为便捷访问点，
    返回最近一次成功初始化的上下文。

    建议在应用启动时初始化一次，之后通过 `get_instance()` 获取最近上下文，
    或持有 `initialize()` 返回的实例引用。
    """
    ...
    def __init__(self) -> None:
        # 每次初始化独立上下文：允许同一个进程中创建多个隔离的容器。
        self._container = None

    @classmethod
    def initialize(
        cls, base_packages: (list[str]) | None = None, config_file: (str) | None = None, enable_aop: bool = True
    ):
        """
        初始化应用上下文
        ...
        if base_packages:
            ...
            if duplicate_packages:
                logger.warning(
                    f"⚠️  用户配置中包含框架包 {duplicate_packages}，已自动去重。框架包会自动扫描，无需手动配置。"
                )
    ...
    def _load_framework_packages(cls) -> list[str]:
        ...
        - 未引入的 starter 不参与扫描。
        - 校验各 starter 声明的 requires 依赖，缺失时明确告警。
        ...
        try:
            ...
            # 校验 requires 依赖，缺失时告警（对标 Spring starter 依赖解析）
            missing = loader.validate_requires(configs)
            for msg in missing:
                logger.warning(f"[autoconfigure] {msg}")

            return [pkg for cfg in configs for pkg in cfg.scan_packages]
