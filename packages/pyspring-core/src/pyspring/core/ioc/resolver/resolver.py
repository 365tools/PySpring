...
from ..registry.registry import ServiceDefinition, ServiceRegistry
...
class DependencyResolver:
    ...
    def _resolve_dependency(
        ...
        # 检查是否会造成循环依赖
        # 统一策略：循环依赖 fail-fast 报错（符合 Spring 默认语义）。
        # 需要延迟代理时请显式使用 @Lazy 注解，而非依赖自动代理掩盖循环。
        if service_name in self._instantiation_stack:
            raise RuntimeError(
                f"检测到循环依赖: {' -> '.join(self._instantiation_stack + [service_name])}。"
                f"请通过构造函数重构消除循环依赖；若确需延迟解析，请对依赖显式标注 @Lazy。"
            )
        ...
        try:
            ...
            try:
                ...
                # Fail-fast：无法解析的依赖（无默认值、非 Optional/Any/内置）直接报错，
                # 避免静默注入 None 掩盖配置错误（符合 Spring 的 fail-fast 语义）
                raise ValueError(
                    f"无法解析依赖 '{dep_info.param_name}'（{current_service} 需要 {dep_info.param_type}）："
                    f"容器中不存在类型为 {dep_info.param_type} 的 Bean，且参数无默认值。"
                    f"请确认已通过 @Component/@Service 注册该服务，或在构造函数中提供默认值。"
                )
