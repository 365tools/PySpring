...
if [[ -z "$PACKAGE_NAME" ]] || [[ "$PACKAGE_NAME" != "pyspring" && "$PACKAGE_NAME" != "pyspring-cli" ]]; then
    echo "用法: $0 <package-name> [test|prod]"
    echo "  package-name: pyspring 或 pyspring-cli"
...
case $PACKAGE_NAME in
    "pyspring")
        TEST_PATHS=("tests/pyspring/")
        ;;
    "pyspring-cli")
        TEST_PATHS=("tests/pyspring_cli/")
        ;;
esac
