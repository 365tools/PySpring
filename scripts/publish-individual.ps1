...
param(
    ...
    [ValidateSet('pyspring', 'pyspring-cli', 'pyspring-core', 'pyspring-web',
                 'pyspring-repositories', 'pyspring-security', 'pyspring-health')]
    [string]$PackageName,
...
$testPaths = @()
switch ($PackageName) {
    "pyspring" { $testPaths = @("tests/") }
    "pyspring-core" { $testPaths = @("tests/core/") }
    "pyspring-web" { $testPaths = @("tests/web/") }
    "pyspring-repositories" { $testPaths = @("tests/repositories/") }
    "pyspring-security" { $testPaths = @("tests/security/") }
    "pyspring-health" { $testPaths = @("tests/health/") }
    "pyspring-cli" { $testPaths = @() }  # CLI 当前无独立测试目录
}
