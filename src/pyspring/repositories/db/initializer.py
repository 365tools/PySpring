...
class DatabaseInitializer(IStartupInitializer):
    ...
    注意：
    - 依赖 DBInitializer 先执行完成（需要数据库连接已建立）
    ...
    async def _get_db_service(self):
        ...
        if self._db_service is None:
            ...
            db_manager = ioc_manager.container.get(
                ioc_manager.generate_name(DBManagerService)
            )
            self._db_service = await db_manager.service()
