...
class CacheInitializer(IStartupInitializer):
    ...
    async def _init_redis(self) -> bool:
        ...
        try:
            # 测试 Redis 连接
            if await self.cache_manager._redis.ping():
                self.cache_manager.ins = self.cache_manager._redis
                self.cache_manager._use_redis = True
                logger.info(f"✅ Redis 缓存已连接: {self.cache_manager._redis.url}")
                return True
            else:
    ...
    async def _init_memory(self) -> bool:
        ...
        """
        self.cache_manager.ins = self.cache_manager._memory
        self.cache_manager._use_redis = False
