...
class HttpResponse(BaseModel, Generic[T]):
    ...
    """
    code: Optional[int] = Field(default=None, description="Business/App code; omitted when equals HTTP status")
    message: Optional[str] = Field(default=None, description="Human-readable message")
    success: Optional[bool] = Field(default=None, description="Indicates if the request was successful")
    data: Optional[T] = Field(default=None, description="Payload object")


def _json(result: HttpResponse, status_code: int | None = None, headers: Optional[Dict[str, str]] = None) -> JSONResponse:
...
class Response:
    ...
    @staticmethod
    def success(result: Any, business_code: Optional[int] = 200, message: Optional[str] = None) -> JSONResponse:
    ...
    @staticmethod
    def error(
            result: HttpResponse | Exception | Any,
        ...
        captured_exc: Optional[Exception] = exc
