...
from pyspring.exception.errors import AppError
from pyspring.exception.handler import GlobalExceptionHandler
from pyspring.log.loguru.context import set_session_id
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    """
    请求日志中间件 - 确保正确的日志顺序
    同时在请求作用域内注入session_id(ContextVar)，便于所有日志包含统一会话标识
    """
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # 计算/注入 session_id(优先Header/Query，其次回退到request_id)
        session_id = (
        ...
        logger.bind(request_id=incoming_id).info(f"🎢 {client_ip} - \"{method} {url}\" - 请求开始")
        ...
        try:
            ...
            # 设置响应头 request id
            try:
                response.headers.setdefault('X-Request-ID', incoming_id)
                # 回写 session id(若需要，可供客户端透传)
            ...
            # 设置响应头
            try:
                error_response.headers.setdefault('X-Request-ID', incoming_id)
                error_response.headers.setdefault('X-Session-ID', session_id)
