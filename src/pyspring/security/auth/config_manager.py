...
class SecurityConfigManager(ISingletonService):
    ...
    _instance: Optional['SecurityConfigManager'] = None
    ...
    def _load_config(self):
        ...
        try:
            ...
            if not config_file:
                print("[SecurityConfigManager] 配置文件不存在")
                print("[SecurityConfigManager] 使用默认配置")
            ...
            print(f"[SecurityConfigManager] 已加载配置文件: {config_file}")

            # 应用环境变量覆盖
            self._apply_env_overrides()

        except Exception as e:
            print(f"[SecurityConfigManager] 加载配置文件失败: {e}")
            print("[SecurityConfigManager] 使用默认配置")
            self._config = self._get_default_config()
    ...
    def _apply_env_overrides(self):
        ...
        if jwt_secret:
            ...
            print("[SecurityConfigManager] JWT_SECRET_KEY 已从环境变量加载")
        ...
        if jwt_encryption_key:
            ...
            print("[SecurityConfigManager] JWT_ENCRYPTION_KEY 已从环境变量加载")
    ...
    def is_device_verification_enabled(self) -> bool:
        """设备验证是否启用"""
        return self.get("authentication.device.enabled", False)
