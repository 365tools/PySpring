...
from pyspring.security.auth.providers.base import AuthProvider, AuthenticationResult, PathMatcher
...
class AuthenticationChain:
    ...
    def register_providers(self, providers: List[AuthProvider]):
        """
        批量注册认证提供者
...
class AuthenticationChainManager(ISingletonService):
    """认证链管理器（由 IoC 容器管理单例）"""

    _chain: Optional[AuthenticationChain] = None

    def __init__(self):
        if self._chain is None:
            self._chain = AuthenticationChain()

    def get_chain(self) -> AuthenticationChain:
        """获取认证链实例"""
        return self._chain
    ...
    def reset(self):
        """重置认证链"""
        if self._chain:
            self._chain.clear_providers()
        self._chain = AuthenticationChain()
        logger.info("🔄 认证链已重置")

# AuthenticationChainManager 现在由 IoC 容器管理，通过容器获取实例
# 示例: container.get(AuthenticationChainManager)
