...
from pyspring.security.auth.models.rabc.orm.tables import UserTable, RoleTable, UserRoleTable, UserDeviceTable
from pyspring.security.auth.models.rabc.schema.requests import UserInfo, User, Role, UserDevice
...
class UserManagerService(ISingletonService):
    ...
    async def update_user_info(self, user_id: int, user_info: UserInfo) -> UserInfo:
        """
        完整更新用户信息（包括基本信息、角色、设备）

        Args:
            user_id: 用户数据库ID
            user_info: 完整的用户信息（包含用户、角色、设备）
        ...
        try:
            async with await self.db.get_session() as session:
                ...
                # 4. 更新用户设备
                if user_info.device is not None:
                    await self._update_user_devices(session, user_id, user_info.device)
    ...
    async def update_user_devices(self, user_id: int, devices: List[UserDevice]) -> UserInfo:
        """
        更新用户的设备列表（替换所有设备）
        ...
        try:
            async with await self.db.get_session() as session:
                ...
                # 更新设备
                await self._update_user_devices(session, user_id, devices)
                await session.commit()

                logger.info(f"✅ 用户设备更新成功: {db_user.email}")
    ...
    async def delete_user(self, user_id: int) -> bool:
        ...
        try:
            async with await self.db.get_session() as session:
                ...
                # 删除用户设备
                stmt = delete(UserDeviceTable).where(UserDeviceTable.user_id == user_id)
                await session.execute(stmt)
    ...
    async def _build_user_info(session: AsyncSession, db_user: UserTable) -> UserInfo:
        ...
        # 查询用户设备
        stmt = select(UserDeviceTable).where(UserDeviceTable.user_id == db_user.id)
        result = await session.execute(stmt)
        devices = [UserDevice.model_validate(device) for device in result.scalars().all()]
    ...
    @staticmethod
    async def _update_user_devices(session: AsyncSession, user_id: int, devices: List[UserDevice]) -> None:
        """
        更新用户设备（替换所有设备）
