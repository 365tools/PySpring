...
from pyspring.security.auth.middleware.device import DeviceCheckMiddleware
from pyspring.security.auth.middleware.role import RoleCheckMiddleware
...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    3. 验证设备指纹（可选）
    4. 验证用户角色权限（可选）
    5. 将用户信息注入到 request.state
    
    类似Spring Boot的拦截器/过滤器
    """

    def __init__(self, app, enable_device_check: bool = None, enable_role_check: bool = None):
        """
        初始化认证中间件
        
        Args:
            app: FastAPI应用实例
            enable_device_check: 是否启用设备验证（None则从配置读取）
        ...
        # 设备验证和角色验证配置（优先使用参数，否则从配置文件读取）
        if enable_device_check is None:
            self.enable_device_check = self.config_manager.is_device_verification_enabled()
        else:
        ...
        # 获取认证链（通过 IoC 容器）
        auth_chain_manager = container.get(AuthenticationChainManager)
        self.auth_chain = auth_chain_manager.get_chain()

        logger.info(f"🔒 全局认证中间件已启动 (基于认证链)")
        logger.info(f"   - 设备验证: {'启用' if self.enable_device_check else '禁用'}")
        logger.info(f"   - 角色验证: {'启用' if self.enable_role_check else '禁用'}")
        logger.info(f"   - 认证提供者: {self.auth_chain.get_provider_count()} 个")
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.enable_role_check:
            ...
            # 注意：原代码中的 role.auth 方法返回 enable_device_check，这里保持兼容
            try:
                device_check_override = await role_middleware.auth(path, request)
                if device_check_override is not None:
                    self.enable_device_check = device_check_override
            except Exception as e:
        ...
        # 5. 设备指纹验证（如果需要）
        if self.enable_device_check:
            device_middleware = DeviceCheckMiddleware()
            device_middleware.enable_device_check = self.enable_device_check
            try:
                payload = auth_result.extra_data or {}
                await device_middleware.auth(path, request, payload)
            except Exception as e:
