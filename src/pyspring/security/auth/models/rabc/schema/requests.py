...
class UserDevice(BaseModel):
    ...
    id: Optional[int] = Field(None, description="数据库主键ID")
    user_id: int = Field(..., description="用户ID")
    # device_code: int = Field(..., description="设备代码")
    name: str = Field(..., description="设备名称")
    fingerprint: str = Field(..., description="设备指纹")
    status: bool = Field(default=True, description="设备状态")
    approved: bool = Field(default=False, description="是否已审批通过")
    expires_at: Optional[str] = Field(None, description="设备权限过期时间")
    requested_duration_days: Optional[int] = Field(default=7, description="申请的访问时长（天）")
...
class UserInfo(BaseModel):
    ...
    device: Optional[list[UserDevice]] = Field(None, description="用户设备列表")
...
class LoginRequest(BaseModel):
    ...
    device_fingerprint: str = Field(default=None, description="设备指纹（由前端生成）")
    device_name: Optional[str] = Field(default=None, description="设备名称（如：Chrome on Windows）")
