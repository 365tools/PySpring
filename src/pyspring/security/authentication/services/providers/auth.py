...
from pyspring.security.authentication.core.interfaces import IAuthenticationProvider, IUserProvider
from pyspring.security.authorization.rabc.schema.requests import LoginRequest


class DefaultPasswordAuthenticationProvider(IAuthenticationProvider):
    """
    ...
    async def authenticate(self, request: Any) -> Any:
        ...
        if not isinstance(request, LoginRequest):
            raise TypeError(f"DefaultPasswordAuthenticationProvider only supports LoginRequest, got {type(request)}")
