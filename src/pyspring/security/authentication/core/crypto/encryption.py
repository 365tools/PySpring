...
class JWTEncryption:
    ...
    def __init__(self, encryption_key: Optional[str] = None, algorithm: str = "Fernet"):
        """
        初始化加密工具
    ...
    def _init_fernet(self):
        ...
        try:
            ...
            logger.info("🔐 Fernet 加密器初始化成功")

        except Exception as e:
            logger.error(f"❌ Fernet 加密器初始化失败: {e}")
    ...
    def _init_aes_gcm(self):
        ...
        try:
            ...
            logger.info("🔐 AES-GCM 加密器初始化成功")

        except Exception as e:
            logger.error(f"❌ AES-GCM 加密器初始化失败: {e}")
...
class JWTEncryptionManager(ISingletonService):
    """
    JWT 加密管理器（由 IoC 容器管理单例）
    
    根据配置自动加载加密设置
    """

    _encryption: Optional[JWTEncryption] = None
    _enabled: bool = False
    ...
    def _load_config(self):
        ...
        if not encryption_key:
            logger.warning("⚠️ JWT 加密已启用，但未提供加密密钥，自动生成临时密钥")
            encryption_key = JWTEncryption.generate_fernet_key()
            logger.warning(f"⚠️ 临时密钥：{encryption_key}")
            logger.warning("⚠️ 生产环境请设置环境变量 JWT_ENCRYPTION_KEY")
        ...
        try:
            ...
            logger.info(f"🔐 JWT 加密已启用 - 算法: {algorithm}")
        except Exception as e:
            logger.error(f"❌ JWT 加密器初始化失败: {e}")
            self._enabled = False
